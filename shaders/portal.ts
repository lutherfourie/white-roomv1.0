...
export const PORTAL_FRAG = `
    ...
    void main() {
        // Map vUv (0..1) to (-1..1) to center the effect
        vec2 uv = vUv * 2.0 - 1.0;
        ...
        color -= 0.2 * vec3(0.0, 0.0, uTime); // Movement through noise
        ...
        // Hardcoded orange palette as requested
        ...
        // Original logic: alphaMask = 1.0 - smoothstep(0.0, 1.0, fac + 1.2);
        // INVERTED Logic: Transparency where color was, Color where transparency was.
        float alphaMask = smoothstep(0.0, 1.0, fac + 1.2);
        
        gl_FragColor = vec4(emissionColor, alphaMask * uOpen);
    }
`;
